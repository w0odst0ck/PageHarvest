...
数据采集的第一步（保存搜索页 HTML）依赖浏览器脚本。已发布到 Greasy Fork，安装后自动翻页保存：

- **震坤行下载器** — 搜索页自动翻页保存 HTML（SPA 适配 + 手动救援）
  → [Greasy Fork 安装](https://greasyfork.org/zh-CN/scripts/586396-%E9%9C%87%E5%9D%A4%E8%A1%8C-html%E4%B8%8B%E8%BD%BD%E5%99%A8)
- **1688 下载器** — 搜索页自动翻页保存 HTML（纯原生 JS，无外部依赖）
  → [Greasy Fork 安装](https://greasyfork.org/zh-CN/scripts/586397-1688-%E8%87%AA%E5%8A%A8%E7%BF%BB%E9%A1%B5-%E4%BF%9D%E5%AD%98)

安装后运行流程：打开搜索页 → 脚本自动开始采集 → HTML 文件下载到本地 → 后续用本框架解析。
